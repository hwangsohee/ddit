...
//jdbc코딩을 위한 mybatis 
//	=> 유지보수 좋아진다. 쿼리를 자바 코드에 안넣어도 됨. try~catch작업들을 줄여서 반복되는 코드를 x 코드 간결-> 가독성이 좋아진다
public class MyBatisTest {
	public static void main(String[] args) {
		...
		try {
			//1-1.설정파일 읽어오기
			Charset charset = Charset.forName("UTF-8"); //한글 처리를 위해서"
			Resources.setCharset(charset);
			
			Reader rd = Resources.getResourceAsReader("config/mybatis-config.xml");
					
			//1-2위에서 생성한 Reader객체를 사용하여 sqlSessionFactory객체 생성하기
		...
		//  ex) 세션객체.insert("namespace값.id값" , 파라미터 객체);
		//		  반환값 : 성공한 레코드 수
		
		/*
		SqlSession session = sessionFactory.openSession(false); // 오토커밋여부 설정
		...
		*/
		...
		SqlSession session = sessionFactory.openSession(); //파라미터 안넣으면 디폴트 false
		...
		} catch(PersistenceException ex) { //mybatis, dao관련 예외처리
			ex.printStackTrace();
			session.rollback();
		} finally {
			session.close();
		...
		try {
			...
			int cnt = session.delete("memberTest.deleteMember", "memberTest.");
		...
		// 이 메서드는 여러개의 레코드를 VO에 담아서 List에 추가해 주는 작업을 자동으로 수행한다.
		...
		try {
			MemberVO mv4 = (MemberVO)session.selectOne("memberTest.getMember", "a0");
